...
exports.nameN = exports.name1 = void 0;
/**
...
modeller.globals.SocketHelper.initialize('sparcsonline.com', 32479);
//turn on debug messages
modeller.globals.PSaaSLogger.getInstance().setLogLevel(modeller.globals.PSaaSLogLevel.VERBOSE //  1 
...
const remotejobsFolder = path_1.default.join(__dirname, './psaas_data/jobs/');
...
var localDir = path_1.default.join(__dirname, './psaas_data/nwt_dataset/test_data/');
...
const sgpModel = (fireID, lat, lon, ele, fireGeom, modelDataset, weather, cffdrs) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        var forecastLines = await weather.records.map((r) => {
        ...
        var spotwxWeatherStream = await spotwxWeatherStation.addWeatherStream(spotwxAttachment, 94.0, 17, modeller.psaas.HFFMCMethod.LAWSON, cffdrs.cffdrs.ffmc, cffdrs.cffdrs.dmc, cffdrs.cffdrs.dc, parseFloat(cffdrs.cffdrs.rn24), weather.firstDateLocal.split(" ")[0], weather.lastDateLocal.split(" ")[0]);
        //    console.log("fireGeom", fireGeom);
        if (fireGeom.geometry.type == 'Point') {
            //create the ignition points
            var latLongObj = await new modeller.globals.LatLon(lat, lon);
            // add it to the model
            // create small fuel poly
            let fuelPoly = await turf_1.default.buffer(fireGeom, 300, { units: 'meters' });
            let coords = await fuelPoly.geometry.coordinates[0].map(pair => {
                return new modeller.globals.LatLon(pair[1], pair[0]);
            });
            var mainIgnition = await prom.addPointIgnition(latLongObj, 
        ...
        //luxon_1.DateTime.fromISO(weather.firstDateLocal.split(" ")[0] + "T13:00:00"),
        scenarioStartString, modeller.globals.Duration.createTime(13, 0, 0, false), 
        //"13:00:00",
        //luxon_1.DateTime.fromISO(weather.firstDateLocal.split(" ")[0] + "T21:00:00"),
        scenarioEndString, modeller.globals.Duration.createTime(13, 0, 0, false), fireID + " Best Case Patch");
        //  "13:00:00");
        ...
        var worstCaseWeatherPatch = prom.addLandscapeWeatherPatch(scenarioStartString, modeller.globals.Duration.createTime(13, 0, 0, false), scenarioEndString, modeller.globals.Duration.createTime(13, 0, 0, false), fireID + " Worst Case Patch");
        ...
        //create 3 scenarios
        let bestScenario = prom.addScenario(scenarioStartString, scenarioEndString, "This is the best case Scenario");
        ...
        bestScenario.setFgmOptions(maxAccTS, distRes, perimRes, minSpreadRos, stopAtGridEnd, breaching, dynSpatialT, spotting, purgeND, useGrowthPctl, GrowthPctl);
        ...
        let bestScenarioOutputKml = prom.addOutputVectorFileToScenario(modeller.psaas.VectorFileType.KML, 'bestCase.kml', scenarioStartString, scenarioEndString, bestScenario);
        ...
        let worstScenario = prom.addScenario(scenarioStartString, scenarioEndString, "This is the worst case Scenario");
        ...
        worstScenario.setFgmOptions(maxAccTS, distRes, perimRes, minSpreadRos, stopAtGridEnd, breaching, dynSpatialT, spotting, purgeND, useGrowthPctl, GrowthPctl);
        ...
        let worstScenarioOutputKml = prom.addOutputVectorFileToScenario(modeller.psaas.VectorFileType.KML, 'worstCase.kml', scenarioStartString, scenarioEndString, worstScenario);
        ...
        if (doThreeDay) {
            let threeDayScenario = prom.addScenario(scenarioStartString, scenarioThreeDayEndString, "This is the Three Day Scenario");
            ...
            threeDayScenario.setFgmOptions(maxAccTS, distRes, perimRes, minSpreadRos, stopAtGridEnd, breaching, dynSpatialT, spotting, purgeND, useGrowthPctl, GrowthPctl);
            ...
            let threeDayScenarioOutputKml = prom.addOutputVectorFileToScenario(modeller.psaas.VectorFileType.KML, '3dayScenario.kml', scenarioStartString, scenarioThreeDayEndString, threeDayScenario);
        ...
        if (prom.isValid()) {
            ...
            const shellOutAndValidate = (shellValidateString, validateFile) => {
                return new Promise(async (resolve, reject) => {
                    await (0, child_process_1.exec)(shellValidateString, async (error, stdout, stderr) => {
                        if (error) {
                        ...
                        if (fs.existsSync(validateFile)) {
                            fs.readFile(validateFile, async (error, bestData) => {
                                if (error) {
                                    modeller.globals.PSaaSLogger.getInstance().info(`error: ${error.message}`);
                                    resolve(false);
                                }
            ...
            await (0, child_process_1.exec)(shellCommandString, async (error, stdout, stderr) => {
                if (error) {
                    modeller.globals.PSaaSLogger.getInstance().info(`error: ${error.message}`);
                    resolve(false);
                }
                ...
                //  console.log(`stdout: ${stdout}`);
                fs.readFile(bestSummaryFile, async (error, bestData) => {
                    if (error) {
                        modeller.globals.PSaaSLogger.getInstance().info(`error: ${error.message}`);
                        resolve(false);
                    }
                        ...
                        // console.log("bestCase:", bestData.toString());
                        fs.readFile(worstSummaryFile, async (error, worstData) => {
                            if (error) {
                                modeller.globals.PSaaSLogger.getInstance().info(`error: ${error.message}`);
                            }
                            else {
...
const manualPsaasExecution = (fireModel) => {
    return new Promise(async (resolve, reject) => {
        modeller.globals.PSaaSLogger.getInstance().info(`Executing PSaaS Directly, ${fireModel.jobName} `);
        // shell out and run psaas
        var jobFile = await remotejobsFolder + fireModel.jobName + "/job.fgmj";
            ...
            fs.readFile(bestSummaryFile, async (error, bestData) => {
                if (error) {
                    ...
                    fs.readFile(worstSummaryFile, async (error, worstData) => {
                        ...
                        // now lets copy the results to serve out
                        fs.copyFile(bestKml, 'data/bestKml.kml', (err) => {
                            if (err)
                                throw err;
                            console.log('copied...', bestKml);
                        });
                        fs.copyFile(worstKml, 'data/worstKml.kml', (err) => {
                            if (err)
                                throw err;
                            console.log('copied...', worstKml);
                        });
                        fs.copyFile(controlKml, 'data/controlKml.kml', (err) => {
                            if (err)
                                throw err;
                            console.log('copied...', controlKml);
                        });
                        fs.copyFile(jobFile, `data/${fireModel.jobName}.fgmj`, (err) => {
                            if (err)
                                throw err;
                            console.log('copied...', jobFile);
                            console.log('grab it here:', `http://psaas-results.api.intellifirenwt.com/data/${fireModel.jobName}.fgmj`);
...
const getFireGeomByFireId = fireId => {
    return new Promise((resolve, reject) => {
        ...
        let urlGps = 'https://geowh.vm.sparcsonline.com/geoserver/sparcs/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=sparcs:current_fire_gps&maxFeatures=1&outputFormat=application/json' +
            cql;
...
const getCentriodFromGeom = (polygon) => {
    ...
    var centroid = turf_1.default.centroid(polygon);
    return centroid;
};
const easySGP = (fireID) => {
    return new Promise(async (resolve, reject) => {
        ...
        const fireCollection = await getFireGeomByFireId(fireID);
...
// }
var spg_1 = require("spg");
Object.defineProperty(exports, "name1", { enumerable: true, get: function () { return spg_1.name1; } });
Object.defineProperty(exports, "nameN", { enumerable: true, get: function () { return spg_1.nameN; } });

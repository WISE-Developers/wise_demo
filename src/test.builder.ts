...
import { globals, client, wise, defaults } from "wise_js_api";
...
try {
    ...
    const liveBuilder = spawn('/usr/bin/java', ['-jar', '/usr/bin/WISE_Builder.jar', '-j', '/root/app_data/wisedemo_jobs/', '&'], {
        detached: true
    });
    ...
    (async function () {
        ...
        const degree_curing = prom.addGridFile(localDir + wiseVersion + '/test/degree_of_curing.asc',
        ...
        const ws = prom.addWeatherStation(1483.0, new globals.LatLon(51.654700, -115.361700));
        const b3Yaha = ws.addWeatherStream(localDir + wiseVersion + '/test/weather_B3_hourly_Sep25toOct30_2001.txt', 94.0, 17,
            wise.HFFMCMethod.LAWSON, 89.0, 58.0, 482.0, 0.0, DateTime.fromISO("2001-09-25"), DateTime.fromISO("2001-10-30"));
        const wpatch = prom.addLandscapeWeatherPatch(DateTime.fromISO("2001-10-16T13:00:00-05:00"), globals.Duration.createTime(13, 0, 0, false),
        ...
        const wpatch2 = prom.addFileWeatherPatch(localDir + wiseVersion + '/test/weather_patch_wd270.kmz', DateTime.fromISO("2001-10-16T13:00:00-05:00"),
        ...
        const ll1 = new globals.LatLon(51.65287648142513, -115.4779078053444);
        const ig3 = prom.addPointIgnition(ll1, DateTime.fromISO('2001-10-16T13:00:00-05:00'));
        const ll2 = new globals.LatLon(51.66090499909746, -115.4086430000001);
        const ig4 = prom.addPointIgnition(ll2, DateTime.fromISO('2001-10-16T16:00:00-05:00'));
        ...
        //create a scenario
        const scen1 = prom.addScenario(DateTime.fromISO('2001-10-16T13:00:00-05:00'), DateTime.fromISO('2001-10-16T22:00:00-05:00'));
        scen1.setName('scen0');
        ...
        const ovf1 = prom.addOutputVectorFileToScenario(wise.VectorFileType.KML, 'scen0/perim.kml', DateTime.fromISO('2001-10-16T13:00:00-05:00'), DateTime.fromISO('2001-10-16T22:00:00-05:00'), scen1);
        ...
        const ogf1 = prom.addOutputGridFileToScenario(globals.GlobalStatistics.TEMPERATURE, 'scen0/temp.txt',
            DateTime.fromISO('2001-10-16T21:00:00-05:00'), wise.Output_GridFileInterpolation.IDW, scen1);
        const ogf2 = prom.addOutputGridFileToScenario(globals.GlobalStatistics.BURN_GRID, "scen0/burn_grid.tif",
            DateTime.fromISO('2001-10-16T22:00:00-05:00'), wise.Output_GridFileInterpolation.IDW, scen1);
        const ogf3 = prom.addOutputGridFileToScenario(globals.GlobalStatistics.TOTAL_FUEL_CONSUMED, 'scen0/total_fuel_consumed.tif',
        ...
        if (errors.length > 0) {
            ...
            const manager = new client.JobManager(jobName);
